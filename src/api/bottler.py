...
def post_deliver_bottles(potions_delivered: list[PotionInventory]):
    ...
    for potion in potions_delivered:
        ...
        used_blue_ml += blue_ml * potion.quantity
    ...
    with db.engine.begin() as connection:
        ...
        connection.execute(sqlalchemy.text(
            "INSERT INTO ml_ledger (color, entry, change, description) VALUES ('blue', 'bottling', :used_blue_ml, 'Bottling operation')"
        ), {"used_blue_ml": -used_blue_ml})
...
def get_bottle_plan():
    with db.engine.begin() as connection:
        potion_info = connection.execute(
            sqlalchemy.text("SELECT potion_id, red_ml, green_ml, blue_ml, dark_ml FROM potion_types WHERE potion_id != 6")
        ).fetchall()
    ...
    while any(ml >= max(potion.red_ml, potion.green_ml, potion.blue_ml, potion.dark_ml) for ml in ml_dict.values() for potion in potion_info) and potion_quantity <= 294:
        ...
        for potion in sorted(potion_info, key=lambda p: potion_dict.get(p.potion_id, 0)):
            ...
            if potion.blue_ml != 0:
                available_blue = ml_dict["blue"] // potion.blue_ml
